...
const MENU_TEMPLATE_DEFAULTS={
  ...
  compact:{template:'compact',background:'#f3f3f3',background_2:'#e9e9e9',background_mode:'solid',surface:'#ffffff',surface_2:'#f2f2f2',surface_mode:'solid',text:'#171717',muted:'#6f6f6f',category_background:'#111111',category_background_2:'#2b2b2b',category_background_mode:'solid',category_text:'#ffffff',gradient_angle:135,card_radius:14,density:'compact',mobile_columns:1,show_hero:true,sticky_categories:true,show_feature_strip:false},
  gallery:{template:'gallery',background:'#f6f1e8',background_2:'#eadcc9',background_mode:'solid',surface:'#fffdf8',surface_2:'#f7ecdc',surface_mode:'solid',text:'#1b1916',muted:'#766f65',category_background:'#fffdf8',category_background_2:'#f1e7d9',category_background_mode:'solid',category_text:'#1b1916',gradient_angle:135,card_radius:30,density:'comfortable',mobile_columns:2,show_hero:true,sticky_categories:true,show_feature_strip:false},
  platecrop:{template:'platecrop',background:'#fff8f1',background_2:'#ffd6bd',background_mode:'gradient',surface:'#fffdf9',surface_2:'#fff0e5',surface_mode:'gradient',text:'#17181b',muted:'#69645f',category_background:'#17181b',category_background_2:'#f26430',category_background_mode:'gradient',category_text:'#ffffff',gradient_angle:135,card_radius:20,density:'comfortable',mobile_columns:2,show_hero:true,sticky_categories:true,show_feature_strip:true}
};
...
function themeControlsTheme(overrides={}){const template=$('.template-card.active')?.dataset.template||'editorial',defaults=MENU_TEMPLATE_DEFAULTS[template]||MENU_TEMPLATE_DEFAULTS.editorial,current=normalizedTheme(currentRestaurant?.menu_theme||{});return {...current,template,background:$('#setMenuBg').value,background_2:$('#setMenuBg2').value,background_mode:$('#setMenuBgMode').value,surface:$('#setMenuSurface').value,surface_2:$('#setMenuSurface2').value,surface_mode:$('#setMenuSurfaceMode').value,text:$('#setMenuText').value,muted:defaults.muted,category_background:$('#setCategoryBg').value,category_background_2:$('#setCategoryBg2').value,category_background_mode:$('#setCategoryBgMode').value,category_text:$('#setCategoryText').value,gradient_angle:Number($('#setGradientAngle').value)||135,card_radius:Number($('#setCardRadius').value)||0,density:$('#setMenuDensity').value,mobile_columns:Number($('#setMobileColumns').value)||1,show_hero:$('#setShowHero').checked,sticky_categories:$('#setStickyCategories').checked,show_feature_strip:$('#setShowFeatureStrip').checked,hero_item_id:$('#setHeroDish')?.value||'',hero_copy:heroCopyFromControls(),...overrides}}
function themeFill(mode,a,b,angle=135){return mode==='gradient'?`linear-gradient(${Number(angle)||135}deg, ${a}, ${b})`:a}
function paintThemePreview(theme=themeControlsTheme()){const preview=$('#themePreview');if(!preview)return;preview.dataset.previewTemplate=theme.template;preview.dataset.previewColumns=String(Number(theme.mobile_columns)||1);preview.style.setProperty('--p-bg',themeFill(theme.background_mode,theme.background,theme.background_2,theme.gradient_angle));preview.style.setProperty('--p-surface',themeFill(theme.surface_mode,theme.surface,theme.surface_2,theme.gradient_angle));preview.style.setProperty('--p-text',theme.text);preview.style.setProperty('--p-cat',themeFill(theme.category_background_mode,theme.category_background,theme.category_background_2,theme.gradient_angle));[['menuBgValue',theme.background],['menuBg2Value',theme.background_2],['menuSurfaceValue',theme.surface],['menuSurface2Value',theme.surface_2],['menuTextValue',theme.text],['categoryBgValue',theme.category_background],['categoryBg2Value',theme.category_background_2],['categoryTextValue',theme.category_text]].forEach(([id,val])=>{const el=$('#'+id);if(el)el.textContent=val});[['setMenuBgMode','setMenuBg2'],['setMenuSurfaceMode','setMenuSurface2'],['setCategoryBgMode','setCategoryBg2']].forEach(([modeId,colorId])=>$('#'+colorId)?.closest('.second-color')?.classList.toggle('inactive-gradient',$('#'+modeId)?.value!=='gradient'))}
function setThemeControls(theme){theme=normalizedTheme(theme);$$('.template-card').forEach(b=>b.classList.toggle('active',b.dataset.template===theme.template));$('#setMenuBg').value=theme.background;$('#setMenuBg2').value=theme.background_2||theme.background;$('#setMenuBgMode').value=theme.background_mode||'solid';$('#setMenuSurface').value=theme.surface;$('#setMenuSurface2').value=theme.surface_2||theme.surface;$('#setMenuSurfaceMode').value=theme.surface_mode||'solid';$('#setMenuText').value=theme.text;$('#setCategoryBg').value=theme.category_background;$('#setCategoryBg2').value=theme.category_background_2||theme.category_background;$('#setCategoryBgMode').value=theme.category_background_mode||'solid';$('#setCategoryText').value=theme.category_text;$('#setGradientAngle').value=String(Number(theme.gradient_angle)||135);$('#setCardRadius').value=String(theme.card_radius);$('#setMenuDensity').value=theme.density;$('#setMobileColumns').value=String(Number(theme.mobile_columns)||1);$('#setShowHero').checked=theme.show_hero!==false;$('#setStickyCategories').checked=theme.sticky_categories!==false;$('#setShowFeatureStrip').checked=theme.show_feature_strip!==false;paintThemePreview(theme)}
...
['setMenuBg','setMenuBg2','setMenuBgMode','setMenuSurface','setMenuSurface2','setMenuSurfaceMode','setMenuText','setCategoryBg','setCategoryBg2','setCategoryBgMode','setCategoryText','setGradientAngle','setCardRadius','setMenuDensity','setMobileColumns','setShowHero','setStickyCategories','setShowFeatureStrip'].forEach(id=>$('#'+id)?.addEventListener('input',()=>paintThemePreview()));
